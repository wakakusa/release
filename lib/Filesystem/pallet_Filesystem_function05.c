...
G_MODULE_EXPORT void create_save_filechooserdialog_OK (GtkWidget *widget,gpointer data  )
{
  ...
  sprintf(Pallet_Filesystem.object_name, "%s", gtk_entry_get_text(Pallet_Filesystem.entry1));//オブジェクト名取得

  Pallet_Filesystem.script1 =g_strconcat("save(",Pallet_Filesystem.object_name,",file=\"",Pallet_Filesystem.file1,"\")\n",NULL);
